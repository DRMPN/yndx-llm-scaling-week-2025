...
def get_cuda_autotune_config():
    return [
        ...
        # custom
        triton.Config({'SPLIT_K': 1, 'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 64,  'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 8}, num_stages=4, num_warps=4),
        triton.Config({'SPLIT_K': 1, 'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 8}, num_stages=4, num_warps=4),
        triton.Config({'SPLIT_K': 1, 'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_N': 256, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 8}, num_stages=4, num_warps=4),
    ]
...
def quantize_int8_perrow_kernel(
    ...
    # !!! FIX: УДАЛЕНО: % M. Теперь используем offs_am < M для маски
    offs_am = pid_m * BLOCK_SIZE_M + tl.arange(0, BLOCK_SIZE_M)
    
    # Маска для строк, чтобы не выходить за пределы M
    mask_m = offs_am < M 
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        # Применяем комбинированную маску для загрузки (строка и столбец)
        fpa = tl.load(fpa_ptrs, mask=mask_m[:, None] & mask_k, other=0.0)
    ...
    # !!! TASK: divide a_max by max positive INT8 value (127)
    # !!! FIX: Гарантируем, что a_max > 1e-6, чтобы избежать деления на 0
    a_scale = tl.maximum(a_max, 1e-6) / 127.0
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        # Применяем комбинированную маску для записи
        tl.store(a_ptrs, inta, mask=mask_m[:, None] & mask_k)
        fpa_ptrs += BLOCK_SIZE_K * stride_fpak
        a_ptrs += BLOCK_SIZE_K * stride_ak
        
    as_offs = pid_m * BLOCK_SIZE_M * stride_asm + tl.arange(0, BLOCK_SIZE_M)
    # Применяем маску для записи scales
    tl.store(as_ptr + as_offs, a_scale, mask=mask_m)
...
def quantize_int8(weight, axis=0, tp_rank=0):
    ...
    max_val = weight.abs().max(dim=axis, keepdim=True)[0]
    # !!! FIX: Гарантируем, что max_val > 1e-6
    scale = torch.clamp_min(max_val, 1e-6) / 127.0

    # !!! TASK: divide weight by scale and convert to int8
    # Теперь деление безопасно, так как scale > 0
...
def perrow_w8a8_matmul_kernel(
    ...
    # Умножение на scales выполнится в float32, а затем результат кастуется в float16 (c_ptr.dtype.element_ty)
